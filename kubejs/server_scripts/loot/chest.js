...
let modifyAndRemoveChestLoot = (/** @type {Internal.LootModificationEventJS} */ event) => {
  event.addLootTypeModifier([LootType.CHEST,LootType.UNKNOWN]).anyDimension(["minecraft:overworld","minecraft:the_nether"]).removeLoot(Ingredient.of(banList))
  event.addLootTypeModifier([LootType.CHEST,LootType.UNKNOWN]).anyDimension(["minecraft:overworld","minecraft:the_nether"]).modifyLoot(Ingredient.of(Object.keys(replacementMap)), (/** @type {Internal.ItemStack} */ item) => Item.of(replacementMap[item.item.id]))
}
