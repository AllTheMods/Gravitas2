...
let loadComponents = (/** @type {Internal.RecipeSchemaRegistryEventJS} */ event) => {
  ...
  keys.result_item = comps.item_stack_provider.key("result_item").defaultOptional().preferred("resultItem")
  ...
  keys.result_fluid = Component("outputFluid").key("result_fluid").defaultOptional().preferred("resultFluid")
  ...
  keys.break_chance = keys.chance.component.key("break_chance").optional(1).alwaysWrite()
  comps.item_registry = new $RegistryComponent(Utils.getRegistry("item"))
  comps.item = builder(comps.item_registry.key("item").defaultOptional())
  comps.item_tag = builder(Component("itemTag").key("tag").defaultOptional())
